...
const ValeStore = (() => {
  ...
  return {
    ...
    addPaciente: async (r) => {
      ...
      const existingIdx = all.findIndex(p => String(p.id) === nr.id);
      if (existingIdx !== -1) {
        all[existingIdx] = nr;
      } else {
        all.unshift(nr);
      ...
      try {
        ...
        console.error('[ValeStore] Erro ao salvar paciente no Supabase:', err);
      }
    ...
    updatePaciente: async (id, changes) => {
      ...
      if (i !== -1) {
        all[i] = { ...all[i], ...changes, id: String(id) };
        if (all[i].name) all[i].name = all[i].name.trim();
        localSave(KEYS.PACIENTES, all);
        try {
          await upsertOne('pacientes', all[i]);
        } catch (err) {
          console.error('[ValeStore] Erro ao atualizar paciente no Supabase:', err);
        }
        dispatchSync('pacientes');
      }
      return all;
    ...
    // Deleção definitiva por ID único: remove dependências por nome e paciente por ID
    deletePaciente: async (idOrName) => {
      ...
      // 1. Atualizar localStorage em cascata (remove pelo ID específico)
      const allPac = pacientes.filter(p => pacId ? String(p.id) !== pacId : (p.name || p.nome) !== pacName);
      ...
      // 2. Deletar dependências no Supabase e deletar o paciente SOMENTE por ID
      const db = getClient();
      if (db) {
        try {
          if (pacName) {
