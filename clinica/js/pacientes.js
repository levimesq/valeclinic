    ...
    if (newPatientForm) {
      newPatientForm.addEventListener('submit', (e) => {
        ...
        if (tbody) {
          ...
          newRow.innerHTML = `
            ...
            <td>
              <div class="action-buttons-group" style="justify-content: flex-end;">
                ...
                <button class="btn-action-icon btn-delete-patient" title="Excluir Paciente" style="color: #EF4444;">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          const btnDel = newRow.querySelector('.btn-delete-patient');
          if (btnDel) {
            btnDel.addEventListener('click', (e) => {
              e.preventDefault();
              e.stopPropagation();
              const confirmed = confirm(`Tem certeza que deseja excluir o cadastro do paciente "${name}"?\n\nEsta ação removerá o paciente permanentemente do sistema e do banco de dados.`);
              if (!confirmed) return;
              newRow.remove();
              updatePatientCounter();
              if (typeof ValeStore !== 'undefined') ValeStore.deletePaciente(newRow.dataset.id || name);
            });
          }
